...
class LearningActivityRepository
{
    ...
    public function saveRecord($request, $id = null)
    {
        ...
        if ( $request->file('attached_file') ) {
            ...
            $med = $this->model->addMediaFromRequest('attached_file')->toMediaCollection('learning_activity_attached_file');


            if (in_array($extension, $allowExtensions)) {              
               // dispatch(new UploadVideoToVimeo($this->model, 'learning_activity_attached_file'));
            } else {
               
                $media = $this->model->getMedia('learning_activity_attached_file');
     ...
     * @return boolean
     */
    public static function canEdit($LearningActivity)
    {
        if (!$user = auth()->user()) {
            return false;
        }
     ...
     * @return boolean
     */
    public static function canReview($LearningActivity)
    {
        if (!$user = auth()->user()) {
            return false;
        }
