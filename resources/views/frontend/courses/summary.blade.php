...
@section('title', str_limit(strip_tags($summary->title), 30) . ' - '. __('Courses'))
                ...
                <div class="row">
                    ...
                    <div class="col-12 col-lg-10 course-content-area">
                        ...
                        <div style="min-height: 240px;">       
                            @if($summaryMedias && count($summaryMedias) > 0)                                                      
                                @include('frontend.courses.partials.include-viewers', ['type' => 'summary'])            
                            @else 
                        ...
                        <div class="text-right mt-1">                         
                            @php 
                                $findVal = $currentSection->lecture_id === null ? 'summary_'.$currentSection->id : 'lsum_'.$currentSection->id; 
                            ...
                            @if($iscompleted)
                                ...
                                @if($isNextSectionAssess) 
                                    <a href="{{ $isReadyToAssess === true ? $nextSection: '#' }}"
                            ...
                            @else
                                {!! \Form::open(array('method' => 'POST', 'route' => 'courses.update-summary-prev-completion', 'class' => 'd-inline' )) !!}
                                    <input type="hidden" name="find_val" value="{{$findVal}}">
                                    <input type="hidden" name="summary_id" value="{{$currentSection->id}}">
                                ...
                                {!! \Form::open(array('method' => 'POST', 'route' => 'courses.update-summary-next-completion', 'class' => 'd-inline' )) !!}
                                    <input type="hidden" name="find_val" value="{{$findVal}}">
                                    <input type="hidden" name="summary_id" value="{{$currentSection->id}}">
                                    <input type="hidden" name="course_id" value="{{$course->id}}">
                                    <input type="hidden" name="user_id" value="{{ auth()->user()->id}}">
                                ...
                                <!-- {!! \Form::open(array('method' => 'POST', 'route' => 'courses.update-summary-completion', 'class' => 'd-inline' )) !!}
                                    <input type="hidden" name="find_val" value="{{$findVal}}">
                                    <input type="hidden" name="summary_id" value="{{$currentSection->id}}">
                                    <input type="hidden" name="course_id" value="{{$course->id}}">
                                    <input type="hidden" name="user_id" value="{{ auth()->user()->id}}">
                                    <button id="summary-mark-complete" class="btn btn-primary btn-sm p-2 mb-2" 
