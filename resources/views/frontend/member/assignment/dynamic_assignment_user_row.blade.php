<tr class="item_row{{$assignment_user->id}}">
    ...
    <td>
        ...
        <button class="edit-modal btn btn-primary" data-value="{{ $assignment_user->comment ? 'Edit Comment' : 'Create Comment' }}" 
        data-id="{{$assignment_user->id}}" data-title="{{$assignment_user->assignment->title}}" data-comment="{{$assignment_user->comment}}">
