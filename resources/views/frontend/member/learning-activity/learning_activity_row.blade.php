@forelse($learningActivities as $key => $learningActivity)
    @if($learningActivity->lecture_id == $lectureId)
        <tr>
            <td>{{ $key + 1 }}</td>
            <td>{!! strip_tags($learningActivity->title) !!}</td>
            <td>@php
                if($learningActivity->lecture_id) {
                    $lecture = App\Repositories\LectureRepository::findById($learningActivity->lecture_id);
                }
                @endphp
                @if(!$learningActivity->lecture_id)
            ...
            <td>{{ $learningActivity->created_at ?? '' }} </td>
            <td class="text-right table-options">
                ...
                <div class="btn-group btn-small">
                        ...
                        @if($canEdit)
                            ...
                            <a class="btn pr-2 pl-2 btn-outline text-info" href="{{ route('member.learning-activity.edit', [$learningActivity->id]) }}" 
                            ...
                            <div class="dropdown-menu">
                                ...
                                @else
                                    {!! Form::open(array('route' => array('member.learning-activity.destroy', $learningActivity->id), 'method' => 'delete'
