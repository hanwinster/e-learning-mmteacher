                        ...
                        <div class="card-header">
                            <h4 class="card-title">
                                @can('add_course_category')
                                    <a href="{{ route('admin.course-type.create') }}" 
                                        class="btn btn-primary btn-md">{{ _('New') }}</a>
                                @endcan    
                            </h4>
                            <div class="card-tools">
                                <form action="{{ route('admin.course-type.index') }}" method="get">
                                    ...
                                    <a href="{{ route('admin.course-type.index') }}" class="btn btn-secondary btn-md">
                                        {{ _('Reset') }}
                                    </a>                                
                                </form>
                            </div>                        
                                        ...
                                        <td class="text-center">
                                            @can('edit_course_category')
                                                <a class="table-action hover-primary cat-edit" 
                                                    href="{{ route('admin.course-type.edit', $post->id) }}" 
                                            ...
                                            @can('delete_course_category')
                                                {!! Form::open(array('route' => array('admin.course-type.destroy', $post->id),
                                                    'method' => 'delete' , 
